* ---------------------------------------------------------------------------
* OutputDataSetsUIMacro_EmpQCEW.do  ->  output/factor_inputs/EmpQCEW/  (tab:Benefits_on_JobCreation
*                                       Employment column)
*
* Exporter for the QCEW-employment IFE. QBLS column contract:
*   col 1  fipsnumeric
*   col 2  year
*   col 3  quarter
*   col 4  diff_logmeanwks       (benefit weeks, first difference; RHS regressor)
*   col 5  qdk1_logqcew_emp      QCEW employment, Uhlig quasi-diff   (var_ind 5)
* Paper Col (3) "Employment": qdk1_logqcew_emp (4 factors).
*
* NOT vacancy-gated -- QCEW county employment is public, so this column is fully
* shippable on either the real or synthetic DataControls vintage.
* ---------------------------------------------------------------------------
...
forvalues i=1/`maxid'{
	disp(`i')
	outsheet fipsnumeric year quarter diff_logmeanwks qdk1_logqcew_emp ///
